...
/**
 * SkeinMain is used to test the SimpleSkein, UBI64 and Threefish
 * implementations and to see that the test vectors, including initial chaining
 * vectors are correct. The output of the main() method has been used to
 * successfully confirm the old and new chaining values as created by the Skein
 * team.
 * <P>
 * The initial implementation incorrectly used the blocksize as size of the
 * configuration encoding instead of the real size of the configuration
 * encoding. You can simulate this incorrect behaviour by simply setting the
 * private USE_BLOCK_SIZE constant to true.
 * <P>
 * The Threefish part of the protocol is of course the most difficult part of
 * the algorithm to implement. To test the intermediate values of your own
 * implementation, just change the logger settings below. For full logging to
 * the console, just set the logging level to Level.FINEST instead of Level.OFF.
 *
 * @author maartenb
 * @author $Author: $
 * @since 5 nov 2008
...
//		System.err.println("doSimpleSkein(135):\n" + org.apache.commons.codec.binary.Hex.encodeHexString(configEncoding) + "\n");
...
//		System.err.println("doSimpleSkein(140):\n" + org.apache.commons.codec.binary.Hex.encodeHexString(blockBuffer) + "\n");
		...
		System.err.println("doSimpleSkein(200):\n" + org.apache.commons.codec.binary.Hex.encodeHexString(output) + "\n");
		return output;
	}
	...
	private static byte[] createTestArray(int bytes) {
		...
		//System.err.println(org.apache.commons.codec.binary.Hex.encodeHexString(testArray));
		return testArray;
	}
...
//		System.out.println();
//
//		System.out.printf("B.2 Skein-256-160%n%n");
...
//		System.out.println();
//
//		System.out.printf("B.10.5 Skein-1024-128%n%n");
//		showConfigurationInit(1024, 128);
//		System.out.println();
//
...
//
		...
		tw.npe.tutorials.Skein512.hash(data, output);
		System.err.println();

		byte[] dummy = new byte[1024];
		String message = "someone@somewhere.net.tw";
		tw.npe.tutorials.Skein512.hash(message.getBytes(), output);
		tw.npe.tutorials.Skein512.hash(message.toUpperCase().getBytes(), output);
		tw.npe.tutorials.Skein512Small.hash(message.getBytes(), dummy);
		tw.npe.tutorials.Skein512Small.hash(message.toUpperCase().getBytes(), dummy);
		message = "oldmanandsea@gmail.com";
		tw.npe.tutorials.Skein512Small.hash(message.getBytes(), dummy);
		tw.npe.tutorials.Skein512Small.hash(message.toUpperCase().getBytes(), dummy);
//
//		// C.1
