...
/**
 * The main class for the Skein hash algorithm by Niels Ferguson, Stefan Lucks,
 * Bruce Schneier, Doug Whiting, Mihir Bellare, Tadayoshi Kohno, Jon Callas and
 * Jesse Walker.
 *
...
public class Skein {
	...
	/**
	 * Calculates the tweak values according to the various tweak fields.
	 * This class is for initial calculations only, it's too slow for
	 * general calculation of tweak values. Warning: due to lsb mode you can
	 * easily confuse tweakHigh and tweakLow. T[0] actually contains the
	 * position (0..2^64 at least) and T[1] the type etc.
	 *
	 * @author maartenb
	 * @author $Author: $
	 * @since 5 nov 2008
	...
	public static class Tweak {
		...
		/**
		 * Convenience constructor that works for values in the range
		 * [0..2^64-1], encoded in the long parameter position as an
		 * unsigned value.
		 *
		 * @param isFinal indicates this is the tweak value of the final
		 * block to be processed
		 * @param isFirst indicates this is the tweak value of the fist
		 * block to be processed
		 * @param type the type contained in the block
		 * @param bitPadded the last byte in the block uses the
		 * indicated bit padding (see class description)
		 * @param treeLevel the level in the tree of this block
		 * @param position the number of bytes processed so far
		 * including the unpadded bytes in this block
		 */
		...
		public Tweak(final boolean isFinal, final boolean isFirst, final int type, final boolean bitPadded, final BigInteger position) {
		...
		/**
		 * Note that because of the LSB configuration of the hash
		 * method, the low tweak value must come to the left of the high
		 * tweak value (visualize the lowest byte containing the lowest
		 * part of the position completely to the left).
		 *
